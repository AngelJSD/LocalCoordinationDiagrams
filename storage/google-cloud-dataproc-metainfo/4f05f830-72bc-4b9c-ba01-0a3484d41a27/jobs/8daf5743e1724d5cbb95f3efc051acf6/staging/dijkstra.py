...
def buildCpd(cell):
  ...
  return (cell.pos[0], cell.pos[1], localGrid[len(localGrid) - 1][len(localGrid) - 1].weight, elapsed)

sc.parallelize(grid.mGrid, 28).flatMap(lambda row: row).map(buildCpd).saveAsTextFile(outputUri+'/debug')
